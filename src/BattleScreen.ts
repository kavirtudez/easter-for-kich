import { Container, type Texture, Sprite, AnimatedSprite } from 'pixi.js'
...
export class BattleScreen extends Container implements IScreen {
  ...
  private eggsPerBattle: number = 5
  ...
  handleAttackClick = (attackType: AttackType): void => {
    ...
    if (queue.length === 0) {
      ...
      if (draggle.health <= 0) {
        ...
        // Add egg finding message after worm faints
        queue.push(() => {
          const currentBattleCount = this.gameState.getBattleCount();
          this.gameState.addEggs(this.eggsPerBattle);
          this.gameState.incrementBattleCount();
          
          // Customize message based on which battle it is
          let eggMessage = `You found ${this.eggsPerBattle} eggs that was hidden by ${draggle.name}!`;
          
          // For the second battle, add a message that no more eggs/battles are available
          if (currentBattleCount === 1) {
            eggMessage += " That was the last one! There are still 5 more eggs to find elsewhere...";
          }
          
          this.showDialogue(eggMessage);
        })
